...
_INJECTED_SECURITY_HEADERS = (
    "X-AgentShield-Data-Trust",
    "X-AgentShield-User-Intent",
    "X-AgentShield-Trace-ID",
    "X-AgentShield-Span-ID",
)
    ...
    def _build_upstream_headers(
        ...
        * Server-computed security headers are injected from metadata.
        ...
        if "data_trust" in metadata:
            headers["X-AgentShield-Data-Trust"] = str(metadata["data_trust"])
        if "user_intent" in metadata:
            headers["X-AgentShield-User-Intent"] = str(metadata["user_intent"])
        if "trace_id" in metadata:
            headers["X-AgentShield-Trace-ID"] = metadata["trace_id"]
        if "span_id" in metadata:
            headers["X-AgentShield-Span-ID"] = metadata["span_id"]
