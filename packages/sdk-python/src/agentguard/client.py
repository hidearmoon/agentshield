...
from agentguard.config import ShieldConfig
from agentguard.exceptions import ServerError
...
class ServerClient:
    ...
    def __init__(self, config: ShieldConfig) -> None:
        ...
        self._http = httpx.AsyncClient(
            ...
            headers={
                ...
                "User-Agent": f"agentguard-python/{_SDK_VERSION}",
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> httpx.Response:
        ...
        try:
            ...
            raise ServerError(f"AgentGuard server timed out ({self._config.base_url}): {exc}") from exc
        except httpx.ConnectError as exc:
            raise ServerError(f"Cannot connect to AgentGuard server ({self._config.base_url}): {exc}") from exc
        except httpx.HTTPError as exc:
            raise ServerError(f"HTTP error: {exc}") from exc

        if resp.status_code >= 500:
            raise ServerError(
                f"AgentGuard server error: {resp.status_code} {resp.text[:200]}",
        ...
        if resp.status_code >= 400:
            ...
            raise ServerError(
                f"AgentGuard request failed ({resp.status_code}): {detail}",
                status_code=resp.status_code,
            )
        return resp
