...
def generate_self_signed_certs(output_dir: str | Path) -> dict[str, Path]:
    ...
    ca_name = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.COMMON_NAME, "AgentGuard Dev CA"),
        ]
    )
    ca_cert = (
    ...
    server_key, server_cert = _issue_cert("agentguard-core", ["localhost", "core", "*.agentguard.local"])
    client_key, client_cert = _issue_cert("agentguard-client", [])
